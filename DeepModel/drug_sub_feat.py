...
class GNNGraph(torch.nn.Module):
    ...
    def forward(self, drug_subs):
        h_node = self.gnn_node(drug_subs)

        h_graph = self.pool(h_node, drug_subs.batch)
        return h_graph
...
class GNN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.virtual_node = args.virtual_node
        self.residual = args.residual
        self.drop_ratio = args.drop_ratio

        self.model = GNNGraph(
            self.num_layer, self.emb_dim, self.gnn_type,
            self.virtual_node, self.residual, self.drop_ratio, self.JK, self.graph_pooling
        )

    def forward(self, drug_subs):
        h_graph = self.model(drug_subs)
        return h_graph
